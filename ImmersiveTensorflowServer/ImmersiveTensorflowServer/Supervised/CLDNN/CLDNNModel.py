...
class CLDNNModel(ModelSkeleton):
  def __init__(self, config : CLDNNConfig):
    self.config = config

    self.input_placeholder = tf.placeholder(tf.float32, [None, config.max_timesteps, config.mfcc_features_count], name="Input__placeholder")
    self.lengths_placeholder = tf.placeholder(tf.int32, [None], name="Lengths_placeholder")
    self.output_placeholder = tf.placeholder(tf.int32, [None, config.max_output_length, config.dictionary_size], name="True_output_placeholder")
        
    self.input_size = config.max_timesteps
    self.output_size = config.max_output_length
  ...
  def inference(self):
    ...
    with tf.name_scope("Input_reduction"):
      ...
      red_time = tf.cast(tf.ceil(self.lengths_placeholder / self.config.time_reduction_factor), tf.int32)
...
def main():
  ...
  with tf.Graph().as_default():
    cldnn_config = CLDNNConfig(150, 40, 22, dictionary_size)
    cldnn = CLDNNModel(input_placeholder, lengths_placeholder, output_placeholder, cldnn_config)
    ...
    session_config.gpu_config.allow_growth = True

    with tf.Session(config = session_config) as session:
      summary_writer = tf.summary.FileWriter('/tmp/custom/CLDNN/logs/', session.graph)
